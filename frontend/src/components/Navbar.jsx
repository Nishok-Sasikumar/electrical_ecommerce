...
function Navbar() {
  ...
  return (
    <div className={`fixed top-6 left-0 right-0 z-50 flex justify-center transition-all duration-500 ${scrolled ? 'top-4' : 'top-6'}`}>
      <nav className={`flex items-center gap-8 px-6 py-3 rounded-full border border-slate-200 dark:border-white/20 shadow-xl transition-all duration-500 bg-white dark:bg-slate-900/95 backdrop-blur-md ${scrolled ? 'px-8 py-2.5 shadow-emerald-500/10 border-primary/20' : 'px-6 py-3'}`}>
        {/* Brand */}
        <Link to="/" className="flex items-center gap-2.5 group">
          ...
          <div className="flex flex-col">
            <span className="font-black text-sm tracking-tight text-slate-900 dark:text-white leading-none">SAI RAM</span>
            <span className="text-[8px] font-black text-primary uppercase tracking-widest">TRADERS</span>
          </div>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-2">
          <Link to="/" className={`px-5 py-3 rounded-full text-sm font-black transition-all leading-none flex items-center ${isActive('/') ? 'bg-primary text-white shadow-glow' : 'text-slate-600 dark:text-slate-300 hover:text-primary dark:hover:text-primary'}`}>
            <span className="pb-0.5 uppercase tracking-widest text-[10px]">{t('nav.home')}</span>
          </Link>
          {user && !user.isAdmin && (
            <>
              <Link to="/catalog" className={`px-5 py-3 rounded-full text-sm font-black transition-all leading-none flex items-center ${isActive('/catalog') ? 'bg-primary text-white shadow-glow' : 'text-slate-600 dark:text-slate-300 hover:text-primary dark:hover:text-primary'}`}>
                <span className="pb-0.5 uppercase tracking-widest text-[10px]">{t('nav.catalog')}</span>
              </Link>
              <Link to="/wishlist" className={`px-5 py-3 rounded-full text-sm font-black transition-all relative leading-none flex items-center ${isActive('/wishlist') ? 'bg-primary text-white shadow-glow' : 'text-slate-600 dark:text-slate-300 hover:text-primary dark:hover:text-primary'}`}>
                <span className="pb-0.5 uppercase tracking-widest text-[10px]">Wishlist</span>
              ...
              <Link to="/cart" className={`px-5 py-3 rounded-full text-sm font-black transition-all relative leading-none flex items-center ${isActive('/cart') ? 'bg-primary text-white shadow-glow' : 'text-slate-600 dark:text-slate-300 hover:text-primary dark:hover:text-primary'}`}>
                <span className="pb-0.5 uppercase tracking-widest text-[10px]">{t('nav.cart')}</span>
                {cartCount > 0 && (
        ...
        <div className="hidden md:flex items-center gap-1.5 bg-slate-100 dark:bg-white/10 p-1.5 rounded-full border border-slate-200 dark:border-white/10 ml-4">
          ...
          <button 
            ...
            className={`min-w-[56px] h-10 px-4 rounded-full text-[11px] font-black transition-all flex items-center justify-center ${i18n.language.startsWith('ta') ? 'bg-white dark:bg-slate-800 text-primary shadow-md' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
          >
            தமிழ்
          </button>
        </div>
            ...
            <div className="flex items-center gap-3">
              <div className="flex flex-col items-end">
                <div className="flex items-center gap-2">
                  {user.isAdmin && (
                    <span className="bg-slate-900 dark:bg-white dark:text-slate-900 text-white dark:text-slate-900 text-[7px] font-black px-1.5 py-0.5 rounded uppercase tracking-tighter border border-white/20 flex-shrink-0">Admin</span>
                  )}
          ...
          <div className="absolute top-full left-0 right-0 mt-4 mx-4 bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-white/10 shadow-2xl p-6 md:hidden flex flex-col gap-4 animate-in slide-in-from-top-4 duration-300">
            ...
            {user ? (
                <div className="flex items-center justify-between px-4 py-2">
                  ...
                  <button onClick={logout} className="text-red-500 font-black uppercase text-[10px] tracking-widest">Logout</button>
                </div>
            ) : (
              <div className="grid grid-cols-2 gap-4">
                <Link to="/login" className="px-6 py-4 rounded-2xl font-bold bg-slate-50 dark:bg-white/5 text-slate-600 dark:text-slate-400 text-center" onClick={() => setIsOpen(false)}>Login</Link>
                <Link to="/signup" className="px-6 py-4 rounded-2xl font-bold bg-primary text-white text-center shadow-glow" onClick={() => setIsOpen(false)}>Sign Up</Link>
              </div>
            )}

...
import { ArrowLeft, View, HelpCircle, Zap, Smartphone, Info, Camera, CameraOff, RefreshCcw, Download, Sliders, Move, RotateCw, Maximize2, AlertCircle, CheckCircle } from "lucide-react"
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            <div className={`text-[10px] font-black px-4 py-2 rounded-full flex items-center gap-2 border tracking-widest uppercase transition-all ${showWebcam ? 'bg-emerald-500 text-white border-emerald-400 shadow-glow' : 'bg-primary/20 text-primary border-primary/20'}`}>
              ...
              {showWebcam ? 'Workspace View Active' : (isSketchfab ? t('ar.interactive', { defaultValue: 'Interactive 3D' }) : t('ar.active', { defaultValue: 'AR Mode Active' }))}
            </div>
          </div>
        </div>
                    ...
                    <div className="absolute top-10 left-1/2 -translate-x-1/2 z-30 flex flex-col items-center gap-4 w-full px-6">
                      {captureStatus ? (
                        ...
                        >
                          ...
                          {showWebcam ? 'Close Workspace View' : (arSupported ? t('ar.view_in_space', { defaultValue: 'View in your space' }) : 'Open Workspace View')}
                        </button>
                      )}

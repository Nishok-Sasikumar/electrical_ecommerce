...
import { ShoppingCart, View, ArrowLeft, ShieldCheck, Truck, RefreshCw, Star, Zap, Package, CheckCircle, Share2, Heart, Check } from "lucide-react"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-start">
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-left-8 duration-700">
            <div className="bg-slate-50 dark:bg-white/5 rounded-[3rem] p-12 overflow-hidden flex items-center justify-center group relative border border-slate-100/50 dark:border-white/10 shadow-soft">
              <img 
                src={mainImage || product.image || "https://via.placeholder.com/600x600?text=No+Image"} 
                alt={product.name}
                ...
                onError={(e) => { e.target.src = "https://via.placeholder.com/600x600?text=Error" }}
              />
          ...
          <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-8 duration-700">
            <div className="mb-12">
              ...
              <div className="flex items-center gap-6 mb-12">
                <p className="text-5xl font-black text-slate-900 dark:text-white tracking-tighter leading-none">₹{product.price}</p>
                <div className="h-10 w-px bg-slate-100 dark:bg-white/10"></div>
                <div>
                  <p className="text-slate-400 dark:text-slate-600 font-bold line-through leading-none mb-1 text-sm">₹{Math.round(product.price * 1.2)}</p>
                  <p className="text-emerald-500 text-xs font-black uppercase tracking-widest">{t('product.flash_deal')}</p>
                </div>
              </div>
              
              <div className="bg-slate-50 dark:bg-white/5 p-10 rounded-[2.5rem] mb-12 border border-slate-100/50 dark:border-white/10 shadow-soft">
                <h3 className="text-lg font-black text-slate-900 dark:text-white mb-4 flex items-center gap-3 tracking-tight">
              ...
              <div className="flex flex-col sm:flex-row gap-6 mb-16">
                <button 
                  ...
                  className="btn-primary h-20 px-12 text-xl flex-grow shadow-glow"
                >
                ...
                {product.arModel && (
                  ...
                  >
                    ...
                    <span className="flex flex-col items-start leading-none">
                      <span className="text-[10px] font-black uppercase tracking-widest mb-1 text-white/80 italic">Live 3D Space</span>
                      {t('product.btn_ar', { defaultValue: 'View in your space' })}
                    </span>
                  </Link>
                )}
              </div>
